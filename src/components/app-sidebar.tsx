          ...
          <div className="relative">
            ...
            >
              ...
              <div className="px-4 pb-2">
                <div className="text-sm text-gray-400">Tweet about...</div>
              </div>
...
export function AppSidebar({ children }: { children: React.ReactNode }) {
  ...
  // Use tRPC instead of client
  const { data: chatHistoryData, isPending: isHistoryPending } = trpc.chat.history.useQuery()
  ...
  // Use tRPC knowledge query
  const { data: knowledgeData } = trpc.knowledge.list.useQuery(undefined, {
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
  })

  const exampleDocuments = knowledgeData?.documents?.filter((doc: { isExample?: boolean }) => doc.isExample) || []
